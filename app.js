...
function renderEwireTransactions() {
    ...
    body.innerHTML = rows.map(item => {
        ...
        return `<tr>
            ...
            <td><strong>${formatEwireNumber(item.receiveAmount)} ${escapeEwireHtml(item.ccy)}</strong><small>Fee: ${formatEwireNumber(item.agentFees)} ${escapeEwireHtml(item.ccy)}</small></td>
            ...
            <td><div class="row-actions"><button type="button" onclick="openEwireWhatsApp('${item.id}')">WhatsApp</button><button type="button" onclick="copySingleEwire('${item.id}')">Copy</button><button type="button" class="danger" onclick="deleteEwireTransaction('${item.id}')">Delete</button></div></td>
        </tr>`;
    }).join('');
}
...
function ewireMessage(item) {
    return [
        ...
        `Date: ${formatEwireDate(item.date)}`,
        `Beneficiary: ${item.beneficiaryName}`,
        `Beneficiary Phone: ${item.beneficiaryPhone}`,
        `Receive Amount: ${formatEwireNumber(item.receiveAmount)} ${item.ccy}`,
        `Agent Fees: ${formatEwireNumber(item.agentFees)} ${item.ccy}`,
        `Country: ${item.country}`,
        `City: ${item.city}`,
        `Agent: ${item.agentName}`,
        `Agent Payment Method: ${item.agentPaymentMethod}`,
        `Pick-Up Location: ${item.pickupLocation || 'N/A'}`,
        `Sender: ${item.senderName}`,
        `Sender Phone: +${item.senderPhone}`,
        `Paid CAD: ${formatEwireNumber(item.paidCad)}`,
        `Paid USD: ${formatEwireNumber(item.paidUsd)}`,
        `Sender Payment Method: ${item.senderPaymentMethod}`,
        `Teller: ${item.teller}`
    ].join('\n');
}
...
function openEwireWhatsApp(id) {
    ...
    if (!item) return;
    if (!item.agentPhone) {
        alert('This agent does not have a WhatsApp phone number.');
        return;
    }
    window.open(`https://wa.me/${item.agentPhone.replace(/\D/g, '')}?text=${encodeURIComponent(ewireMessage(item))}`, '_blank', 'noopener');
}
...
const EWIRE_EXPORT_COLUMNS = [
    ...
    ['Receive Amount', 'receiveAmount'], ['CCY', 'ccy'], ['Agent Fees', 'agentFees'], ['Country', 'country'], ['City', 'city'],
    ...
    ['Sender Payment Method', 'senderPaymentMethod'], ['Teller', 'teller'], ['Synced', 'synced']
];
...
function copyEwireForSheets() {
    ...
    const tsv = [EWIRE_EXPORT_COLUMNS.map(column => column[0]), ...rows.map(item => EWIRE_EXPORT_COLUMNS.map(column => item[column[1]]))]
...
function downloadEwireCsv() {
    ...
    const csv = [EWIRE_EXPORT_COLUMNS.map(column => column[0]), ...rows.map(item => EWIRE_EXPORT_COLUMNS.map(column => item[column[1]]))]
...
async function sendEwireToGoogleSheet() {
    ...
    try {
        await fetch(url, {
            ...
            body: JSON.stringify({ action: 'appendTransactions', transactions: pending })
        });
