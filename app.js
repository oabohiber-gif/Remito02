...
// --- Data Model ---
        ...
        function renderReceiveCurrencyOptions(country) {
            const toggle = document.getElementById('recipient-currency-toggle');
            if (!toggle) return;
            if (!country) {
            ...
            toggle.innerHTML = supported.map(currency => {
                const selected = currency === receiveCurrency;
                const single = supported.length === 1;
                return `<button type="button" class="toggle-btn${selected ? ' active' : ''}" data-cur="${currency}" onclick="setReceiveCurrency('${currency}')" ${single ? 'disabled' : ''}>${flagImage(RECEIVE_CURRENCY_FLAGS[currency], currency + ' flag')} ${currency}</button>`;
            }).join('');
            toggle.style.display = 'flex';
        }  // 'recipient' or 'send' — which box the user last typed into
        ...
        function setReceiveCurrency(cur) {
            ...
            if (!getReceiveCurrencies(country).includes(cur) || cur === receiveCurrency) return;
            receiveCurrency = cur;
...
// Cosmetic: keep the ticket route strip in sync with the selected corridor.
        ...
        document.querySelectorAll('#recipient-currency-toggle .toggle-btn').forEach(btn => btn.addEventListener('click', updateTicketDestination));
        updateTicketDestination();
