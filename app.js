...
function ewireSenderMessage(item) {
    const paidLines = [];
    if (Number(item.paidCad) > 0) paidLines.push(`Paid Amount: ${formatEwireNumber(item.paidCad)} CAD`);
    if (Number(item.paidUsd) > 0) paidLines.push(`Paid Amount: ${formatEwireNumber(item.paidUsd)} USD`);
    return [
        `*Remito eWire Confirmation*`,
        ...
        `Pick-Up Location: ${item.pickupLocation || 'To be confirmed'}`,
        ...paidLines,
        `Payment Method: ${item.senderPaymentMethod}`,
        ``,
...
function getEwireMessage(item, target) {
    if (target === 'sender') return item.senderMessage || ewireSenderMessage(item);
    return item.agentMessage || ewireAgentMessage(item);
}
