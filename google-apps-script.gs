...
const EWIRE_HEADERS = [
  ...
  'CCY', 'Agent Fees (USD)', 'Country', 'City', 'Agent', 'Agent Phone',
  ...
  'Agent WhatsApp Message', 'Sender WhatsApp Message', 'Record ID', 'Received by Sheet At'
];
...
function doPost(e) {
  try {
    ...
    const existingIds = sheet.getLastRow() > 1
      ? new Set(sheet.getRange(2, 22, sheet.getLastRow() - 1, 1).getDisplayValues().flat())
      : new Set();
    ...
    const rows = payload.transactions
      ...
      .map(item => [
        ...
        item.senderPaymentMethod || '', item.teller || '',
        item.agentMessage || '', item.senderMessage || '', item.id, new Date()
      ]);
